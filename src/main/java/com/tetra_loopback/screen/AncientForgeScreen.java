package com.tetra_loopback.screen;
...
public class AncientForgeScreen extends AbstractContainerScreen<AncientForgeMenu> {
    ...
    private static final int CRAFT_U = 176;    // 进度条纹理U坐标
    private static final int CRAFT_V = 14;     // 进度条纹理V坐标
    ...
    protected void renderBg(GuiGraphics gui, float partialTick, int mouseX, int mouseY) {
        ...
        if (craftTime > 0 && craftTimeTotal > 0) {
            ...
            int progressWidth = (int) (CRAFT_WIDTH * progress);

            if (progressWidth > 0) {
                int screenX = guiLeft + CRAFT_X;
                int screenY = guiTop + CRAFT_Y;

                gui.blit(
                        TEXTURE,
                        screenX, screenY,
                        CRAFT_U, CRAFT_V,
                        progressWidth, CRAFT_HEIGHT,
                        256, 256
                );
            }
        }
