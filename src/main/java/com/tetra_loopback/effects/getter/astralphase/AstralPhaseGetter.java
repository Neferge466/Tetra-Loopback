...
public class AstralPhaseGetter implements IStatGetter {
    private final IStatGetter effectLevelGetter;

    public AstralPhaseGetter() {
        this.effectLevelGetter = new StatGetterEffectLevel(ModEffectStats.astralPhaseEffect);
    }

    @Override
    public boolean shouldShow(Player player, ItemStack currentStack, ItemStack previewStack) {
        return hasAstralPhase(currentStack) || hasAstralPhase(previewStack);
    }

    private boolean hasAstralPhase(ItemStack itemStack) {
