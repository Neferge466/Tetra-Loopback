...
public class TelluricAnchorGetter implements IStatGetter {
    private final IStatGetter effectLevelGetter;

    public TelluricAnchorGetter() {
        this.effectLevelGetter = new StatGetterEffectLevel(ModEffectStats.telluricAnchorEffect);
    }

    @Override
    public boolean shouldShow(Player player, ItemStack currentStack, ItemStack previewStack) {
        return hasTelluricAnchor(currentStack) || hasTelluricAnchor(previewStack);
    }

    private boolean hasTelluricAnchor(ItemStack itemStack) {
