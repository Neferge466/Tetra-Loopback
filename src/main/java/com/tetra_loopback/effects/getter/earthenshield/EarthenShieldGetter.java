...
public class EarthenShieldGetter implements IStatGetter {
    private final IStatGetter effectLevelGetter;

    public EarthenShieldGetter() {
        this.effectLevelGetter = new StatGetterEffectLevel(ModEffectStats.earthenShieldEffect);
    }

    @Override
    public boolean shouldShow(Player player, ItemStack currentStack, ItemStack previewStack) {
        return hasEarthenShield(currentStack) || hasEarthenShield(previewStack);
    }

    private boolean hasEarthenShield(ItemStack itemStack) {
